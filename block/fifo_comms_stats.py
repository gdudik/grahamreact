...
## THRESHOLD
ACCEL_THRESHOLD_MEN = micropython.const(3)
ACCEL_THRESHOLD_WOMEN = micropython.const(8)
HYST = micropython.const(0.05)
...
# Initialize stats storage at module level
# _timing_stats = {}
...
#     if myname not in _timing_stats:
#         _timing_stats[myname] = {'count': 0, 'total': 0, 'min': float('inf'), 'max': 0}
...
#         # Update stats
#         stats = _timing_stats[myname]
#         stats['count'] += 1
#         stats['total'] += delta_ms
#         stats['min'] = min(stats['min'], delta_ms)
#         stats['max'] = max(stats['max'], delta_ms)
...
#                 print('  Calls: {}, Average: {:6.3f}ms, Min: {:6.3f}ms, Max: {:6.3f}ms'.format(
...
def check_false_start(impulse, timestamp):
    ...
    if not in_window and prev_impulse < ACCEL_THRESHOLD <= impulse:
       in_window = True
    ...
    if in_window and not runner_started:
        if impulse > prev_impulse + HYST:
            ...
            if impulse < (ACCEL_THRESHOLD - HYST):
                in_window = False
                reaction_time_timestamp = None
                rising_count = 0
        ...
        if rising_count >= RISE_STREAK_N:
            ...
            if runner_started and not gun_triggered: #rt is pre-gun
                ...
                if (reaction_time_timestamp - gun_timestamp) * 0.00003052 < 0.1: # type: ignore #pre calculated 1/32768 for rtc timestamps
                    fs_alert.value(1) 
...
def setup():
    ...
    global gun_timestamp, runner_started_ts
    gun_timestamp = None
    runner_started_ts = None
    fs_alert.value(0)
    create_buffer()
    ...
    imu.write_reg(0x52, 0b11100000) # GYRO_ACCEL_CONFIG0: ACCEL_UI_FILT_BW= 6, GYRO_UI_FILT_BW= 0
...
def start_loop():
    ...
    gc.collect()
    int_counter = 0
    if gun_timestamp and reaction_time_timestamp:
